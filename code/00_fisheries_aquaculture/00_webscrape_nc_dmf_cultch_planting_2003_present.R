################################################################################
### 0. Webscrape Data -- North Carolina DMF cultch planting (2003 - present) ###
################################################################################
...
# Navigates to North Carolina DMF cultch planting (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::planting-sites-2003-present)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::planting-sites-2003-present"
...
file.rename(from=file.path(download_dir, "CultchApp2022.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "nc_cultch_2003_present.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "nc_cultch_2003_present.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "nc_cultch_2003_present.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "nc_cultch_2003_present.zip"))
