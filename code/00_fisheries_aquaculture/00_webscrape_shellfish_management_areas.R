#######################################################
### 0. Webscrape Data -- shellfish management areas ###
#######################################################
...
# Navigates to shellfish management areas (source: https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::shellfish-management-areas)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::shellfish-management-areas"
...
file.rename(from=file.path(download_dir, "Shellfish_Management_Areas.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Shellfish_Management_Areas.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "Shellfish_Management_Areas.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "Shellfish_Management_Areas.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "Shellfish_Management_Areas.zip"))
