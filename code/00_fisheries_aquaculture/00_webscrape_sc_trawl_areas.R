#######################################################
### 0. Webscrape Data -- South Carolina trawl areas ###
#######################################################
...
# Navigates to South Carolina trawl area boundaries (source: https://data-scdnr.opendata.arcgis.com/datasets/81d3a61bf2854f38b5e52e6b7c005b62_0)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-scdnr.opendata.arcgis.com/datasets/81d3a61bf2854f38b5e52e6b7c005b62_0"
...
file.rename(from=file.path(download_dir, "SC_Official_Trawl_Area_Boundaries.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "SC_Official_Trawl_Area_Boundaries.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "SC_Official_Trawl_Area_Boundaries.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "SC_Official_Trawl_Area_Boundaries.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "SC_Official_Trawl_Area_Boundaries.zip"))
