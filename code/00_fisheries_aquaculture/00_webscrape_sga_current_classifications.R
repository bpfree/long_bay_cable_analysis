###################################################################
### 0. Webscrape Data -- North Carolina shellfish growing areas ###
###################################################################
...
# Navigates to North Carolina shellfish growing areas (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::sga-current-classifications-4)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::sga-current-classifications-4"
...
file.rename(from=file.path(download_dir, "SGA_Current_Classifications.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "SGA_Current_Classifications.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "SGA_Current_Classifications.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "SGA_Current_Classifications.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "SGA_Current_Classifications.zip"))
