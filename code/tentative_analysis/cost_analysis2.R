...
terra::ext(pelagic_birds) <- terra::ext(gom_raster)
...
terra::ext(bathymetry) <- terra::ext(gom_raster)

### Slope
slope <- terra::rast(paste(raster_dir, "slope_normalize.grd", sep = "/"))
terra::ext(slope) <- terra::ext(gom_raster)
...
arc_gis_cost_raster <- c(cost_raster,
                         constraints) %>%
...
## Inspect new raster
minmax(cost_raster)[1,] # 0.5
minmax(cost_raster)[2,] # maximum value = 6.162601
