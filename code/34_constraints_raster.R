...
# constraints when active oil and gas lease areas are treated as easements
constraints_active_lease_removed <- seagrass %>%
  # combine constraints datasets so that each is an unique row
  rbind(oyster,
        conservation_area,
        artificial_reef,
...
constraints <- terra::cover(x = seagrass_raster,
  ...
  # cover with environmental sensor raster
  terra::cover(y = environmental_sensor_raster)

#####################################
## constraints when active oil and gas lease areas are treated as easements
constraints_minus_active_lease <- terra::cover(x = seagrass_raster,
                                               y = oyster_raster) %>%
  # cover with conservation area raster
  terra::cover(y = conservation_area_raster) %>%
  # cover with artificial reef raster
...
constraints_minus_active_lease[constraints_minus_active_lease == 0] <- 99
...
sf::st_write(obj = constraints_active_lease_removed, dsn = least_cost_gpkg, "constraints_active_lease_removed", append = F)

## Raster data
terra::writeRaster(constraints, filename = file.path(least_cost_dir, "constraints_raster.grd"), overwrite = T)
terra::writeRaster(constraints_minus_active_lease, filename = file.path(least_cost_dir, "constraints_minus_active_lease.grd"), overwrite = T)
