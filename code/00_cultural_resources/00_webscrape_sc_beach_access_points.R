################################################
### 0. Webscrape Data -- beach access points ###
################################################
...
# Navigates to South Carolina beache access points (source: https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::beach-access-points)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::beach-access-points"
...
file.rename(from=file.path(download_dir, "Beach_Access_Points.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Beach_Access_Points.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "Beach_Access_Points.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "Beach_Access_Points.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "Beach_Access_Points.zip"))
