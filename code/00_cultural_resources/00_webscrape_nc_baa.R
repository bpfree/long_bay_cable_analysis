################################################################
### 0. Webscrape Data -- North Carolina boating access areas ###
################################################################
...
# Navigates to North Carolina boating access areas (source: https://www.nconemap.gov/datasets/ncwrc::baa/explore)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://www.nconemap.gov/datasets/ncwrc::baa/explore"
...
file.rename(from=file.path(download_dir, "BAA.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "BAA.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "BAA.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "BAA.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "BAA.zip"))
