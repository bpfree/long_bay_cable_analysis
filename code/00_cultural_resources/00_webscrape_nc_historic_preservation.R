#################################################################
### 0. Webscrape Data -- North Carolina historic preservation ###
#################################################################
...
# Navigates to North Carolina historic preservation (source: https://www.hpo.nc.gov/gis/nchpogisdata2024-03-04/open)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://www.hpo.nc.gov/gis/nchpogisdata2024-03-04/open"
...
file.rename(from=file.path(download_dir, "NCHPO_GISdata_2024-03-04.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "NCHPO_GISdata.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "NCHPO_GISdata.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "NCHPO_GISdata.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "NCHPO_GISdata.zip"))
