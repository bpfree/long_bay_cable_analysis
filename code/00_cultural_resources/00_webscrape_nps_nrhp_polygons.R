##########################################################################
### 0. Webscrape Data -- National Register of Historic Places polygons ###
##########################################################################
...
# Navigates to National Register of Historic Places (polygons) (source: https://public-nps.opendata.arcgis.com/datasets/nps::national-register-of-historic-places-polygons)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://public-nps.opendata.arcgis.com/datasets/nps::national-register-of-historic-places-polygons"
...
file.rename(from=file.path(download_dir, "NPS_-_National_Register_of_Historic_Places_Locations.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "nps_nrhp_polygons.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "nps_nrhp_polygons.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "nps_nrhp_polygons.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "nps_nrhp_polygons.zip"))
