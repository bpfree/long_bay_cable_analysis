#############################################
### 0. Webscrape Data -- RCCP communities ###
#############################################
...
# Navigates to North Carolina RCCP communities (all phases) (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::rccp-communities-all-phases)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::rccp-communities-all-phases"
...
file.rename(from=file.path(download_dir, "RCCP_Communities_-_All_Phases.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "rccp_communities.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "rccp_communities.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "rccp_communities.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "rccp_communities.zip"))
