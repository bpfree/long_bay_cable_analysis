##########################################################
### 0. Webscrape Data -- intermodal freight facilities ###
##########################################################
...
# Navigates to Intermodal Freight Facilities Marine Roll-on/Roll-off (source: https://data-usdot.opendata.arcgis.com/datasets/usdot::intermodal-freight-facilities-marine-roll-on-roll-off/about)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-usdot.opendata.arcgis.com/datasets/usdot::intermodal-freight-facilities-marine-roll-on-roll-off/about"
...
### geodatabase are the fifth dataset (after CSV, KML, shapefile, and geojson)
geodatabase_panel_button[[5]]$clickElement()

### generate latest data
geodatabase_latest_data <- shadowr::find_elements(shadow_rd, 'calcite-dropdown-item')
### click the option for the latest data for the geodatabase
geodatabase_latest_data[[10]]$clickElement()
#### data are large so need to have system have the the time to download before closing
Sys.sleep(2)
...
file.rename(from=file.path(download_dir, "Intermodal_Freight_Facilities_Marine_Roll-on_Roll-off.gdb.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Intermodal_Freight_Facilities.gdb.zip"))

#####################################

## WARNING: Will have to manual unzip the file as the new names are very strange

file.rename(from=file.path(data_dir, "_ags_dataF1DFA6ADF8D64CC7861F8C301DE03774.gdb"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "intermodal_freight_facilities.gdb"))

## remove original zipped file
file.remove(file.path(data_dir, "Intermodal_Freight_Facilities.gdb.zip"))
