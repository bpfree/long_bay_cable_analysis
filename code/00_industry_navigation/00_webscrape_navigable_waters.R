################################################
### 0. Webscrape Data -- navigable waterways ###
################################################
...
# Navigates to navigable waterway network lines (source: https://geodata.bts.gov/datasets/usdot::navigable-waterway-network-lines/explore)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://geodata.bts.gov/datasets/usdot::navigable-waterway-network-lines/explore"
...
####          is for the CSV, hence the 6th element is for the geodatabase
geodatabase_panel_button[[6]]$clickElement()
...
file.rename(from=file.path(download_dir, "Navigable_Waterway_Network_Lines.gdb.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Navigable_Waterway_Network_Lines.gdb.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "Navigable_Waterway_Network_Lines.gdb.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "Navigable_Waterway_Network_Lines.gdb.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "Navigable_Waterway_Network_Lines.gdb.zip"))

# rename geodatabase
file.rename(from = file.path(paste(data_dir, "Navigable_Waterway_Network_Lines.gdb", sep = "/"),
                           "3ef9be0c-83c7-42ab-8310-18fbbdb37e21.gdb"),
            # send to the same location (just change the name of the geodatabase)
            to = file.path(paste(data_dir, "Navigable_Waterway_Network_Lines.gdb", sep = "/"),
                           "Navigable_Waterway_Network_Lines.gdb"))
