##########################################
### 0. Webscrape Data -- subcatchments ###
##########################################
...
# Navigates to subcatchments (source: https://data.virginiabeach.gov/datasets/03d643ba7ea34fef9f8798473d79c245_0)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data.virginiabeach.gov/datasets/03d643ba7ea34fef9f8798473d79c245_0"
...
file.rename(from=file.path(download_dir, "Subcatchments.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Subcatchments.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "Subcatchments.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "Subcatchments.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "Subcatchments.zip"))
