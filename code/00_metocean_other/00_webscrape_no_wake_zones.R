##########################################
### 0. Webscrape Data -- no wake zones ###
##########################################
...
# Navigates to no wake zones (source: https://www.nconemap.gov/datasets/0ba8a9f778c143c6b71b2848e8ef66e3_0)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://www.nconemap.gov/datasets/0ba8a9f778c143c6b71b2848e8ef66e3_0"
...
file.rename(from=file.path(download_dir, "No-Wake_Zones.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "no_wake_zones.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "no_wake_zones.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "no_wake_zones.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "no_wake_zones.zip"))
