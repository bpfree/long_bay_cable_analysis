...
# Navigates to South Carolina TMDL sites (source: https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::approved-tmdl-sites)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::approved-tmdl-sites"
...
file.rename(from=file.path(download_dir, "Approved_TMDL_Sites.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "tmdl_sites.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "tmdl_sites.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "tmdl_sites.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "tmdl_sites.zip"))
