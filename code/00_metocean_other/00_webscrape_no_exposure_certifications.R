##############################################
### 0. Webscrape Data -- No exposure sites ###
##############################################
...
# Navigates to no exposure sites (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::no-exposure-certifications)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::no-exposure-certifications"
...
file.rename(from=file.path(download_dir, "NO_EXPOSURE_Certifications.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "no_exposure_certs.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "no_exposure_certs.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "no_exposure_certs.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "no_exposure_certs.zip"))
