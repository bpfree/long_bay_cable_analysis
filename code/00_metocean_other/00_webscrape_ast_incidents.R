##########################################
### 0. Webscrape Data -- AST incidents ###
##########################################
...
# Navigates to AST incidents (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::ast-incidents/)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::ast-incidents/"
...
file.rename(from=file.path(download_dir, "AST_Incidents.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "ast_incidents.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "ast_incidents.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "ast_incidents.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "ast_incidents.zip"))
