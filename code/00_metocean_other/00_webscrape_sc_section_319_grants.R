#######################################################
### 0. Webscrape Data -- Section 319 grant projects ###
#######################################################
...
# Navigates to Section 319 grant projects (source: https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::section-319-grant-projects)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::section-319-grant-projects"
...
file.rename(from=file.path(download_dir, "Section_319_Grant_Projects.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "section319_projects.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "section319_projects.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "section319_projects.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "section319_projects.zip"))
