###################################################################
### 0. Webscrape Data -- water quality monitoring plan stations ###
###################################################################
...
# Navigates to water quality monitoring plan stations (source: https://geohub-vadeq.hub.arcgis.com/datasets/fcbf74731d4d4c79a881f8dd23587e52_131)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://geohub-vadeq.hub.arcgis.com/datasets/fcbf74731d4d4c79a881f8dd23587e52_131"
...
file.rename(from=file.path(download_dir, "Water_Quality_Monitoring_Plan_Stations_(Current).zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "water_quality_monitoring_stations.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "water_quality_monitoring_stations.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "water_quality_monitoring_stations.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "water_quality_monitoring_stations.zip"))
