########################################################
### 0. Webscrape Data -- South Carolina storm tracks ###
########################################################
...
# Navigates to South Carolina storm tracks (source: https://data-scdnr.opendata.arcgis.com/datasets/scdnr::scstormtracks-1)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-scdnr.opendata.arcgis.com/datasets/scdnr::scstormtracks-1"
...
file.rename(from=file.path(download_dir, "SC_Hurricanes_Public.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "SC_Hurricanes_Public.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "SC_Hurricanes_Public.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "SC_Hurricanes_Public.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "SC_Hurricanes_Public.zip"))
