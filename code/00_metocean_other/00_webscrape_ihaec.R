###################################################################################
### 0. Webscrape Data -- DCM inlet hazard area of environmental concern (IHAEC) ###
###################################################################################
...
# Navigates to DCM inlet hazard area of environmental concern (IHAEC) (source: https://ncdenr.maps.arcgis.com/home/item.html?id=4acf38b9dfd4479298de0f6b07832558)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://ncdenr.maps.arcgis.com/home/item.html?id=4acf38b9dfd4479298de0f6b07832558"
...
file.rename(from=file.path(download_dir, "iha.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "iha.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "iha.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "iha.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "iha.zip"))
