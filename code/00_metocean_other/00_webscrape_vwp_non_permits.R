############################################
### 0. Webscrape Data -- VWP non-permits ###
############################################
...
# Navigates to VWP non-permits (source: https://geohub-vadeq.hub.arcgis.com/datasets/3daa69f3e3a741599bd85e3725a82acb_191)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://geohub-vadeq.hub.arcgis.com/datasets/3daa69f3e3a741599bd85e3725a82acb_191"
...
file.rename(from=file.path(download_dir, "VWP_Non-Permit_(NP).zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "vwp_non_permits.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "vwp_non_permits.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "vwp_non_permits.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "vwp_non_permits.zip"))
