#########################################################
### 0. Webscrape Data -- North Carolina NPDES permits ###
#########################################################
...
# Navigates to North Carolina NPDES stormwater permits (source: https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::npdes-stormwater-permits)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-ncdenr.opendata.arcgis.com/datasets/ncdenr::npdes-stormwater-permits"
...
file.rename(from=file.path(download_dir, "NPDES_Stormwater_Permits.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "nc_npdes_permits.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "nc_npdes_permits.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "nc_npdes_permits.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "nc_npdes_permits.zip"))
