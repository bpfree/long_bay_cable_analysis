################################################
### 0. Webscrape Data -- VWP general permits ###
################################################
...
# Navigates to VWP general permits (source: https://geohub-vadeq.hub.arcgis.com/datasets/04dfc61324a5406b8fa87fd6184b1faf_193)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://geohub-vadeq.hub.arcgis.com/datasets/04dfc61324a5406b8fa87fd6184b1faf_193"
...
file.rename(from=file.path(download_dir, "VWP_General_Permits_(GP).zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "vwp_general_permits.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "vwp_general_permits.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "vwp_general_permits.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "vwp_general_permits.zip"))
