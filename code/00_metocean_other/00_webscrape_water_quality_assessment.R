###########################################################################
### 0. Webscrape Data -- 2022 Water Quality Assessments and 303(d) list ###
###########################################################################
...
# Navigates to 2022 Water Quality Assessments and 303(d) list (source: https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::2022-water-quality-assessments-and-303d-list)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://sc-department-of-health-and-environmental-control-gis-sc-dhec.hub.arcgis.com/datasets/SC-DHEC::2022-water-quality-assessments-and-303d-list"
...
file.rename(from=file.path(download_dir, "2022_Water_Quality_Assessments_and_303(d)_list.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "water_quality_assessment.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "water_quality_assessment.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "water_quality_assessment.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "water_quality_assessment.zip"))
