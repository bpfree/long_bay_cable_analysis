####################################################
### 0. Data layer clip loop -- Carolinas (extra) ###
####################################################
...
carolinas_extra_gdb <- "data/a_raw_data/Carolinas_extras.gdb"

## export data geopackage
data_gpkg <- "data/b_intermediate_data/carolinas_extra.gpkg"
...
sf::st_layers(dsn = carolinas_extra_gdb,
              do_count = T)
...
for(i in 1:length(vector)){ # use length(sf::st_layers(dsn = marine_cadastre_gdb, do_count = T)[[1]]) if all data are vector data
  ...
  if(i == 117){
    ...
    data_name <- paste(sf::st_layers(dsn = carolinas_extra_gdb,
                                     do_count = T)[[1]][j], "clip", sep = "_")
    
    # load data and clip
    try(data_set <- sf::st_read(dsn = carolinas_extra_gdb,
                                layer = sf::st_layers(carolinas_extra_gdb)[[1]][j]) %>%
  ...
  if(!(i == 117 || i == 118 || i == 119)){
    ...
    # load data and clip
    try(data_set <- sf::st_read(dsn = carolinas_extra_gdb,
                                layer = sf::st_layers(carolinas_extra_gdb)[[1]][i]) %>%
          # reproject data to match study region coordinate reference system
          sf::st_transform(x = .,
                           # coordinate reference system (EPSG:5070, NAD83 / Conus Albers)
                           crs = crs) %>%
